...
def frame_extractor(camera_index=0, width=1280, height=720, output_dir="../camera_captures"):
    """
    Capture frames from a camera and save them to disk on user command.

    A live camera feed is displayed in a window. Press:
    - 'g' to capture and save the current frame
    - 'q' or ESC to exit the application

    Parameters
    ----------
    camera_index : int, optional
        Index of the camera to use (default is 0).
    width : int, optional
        Desired frame width in pixels (default is 1280).
    height : int, optional
        Desired frame height in pixels (default is 720).
    ...
    Notes
    -----
    - Captured images are saved in JPG format using a timestamp-based
      filename to ensure uniqueness.
    - The displayed frame is horizontally flipped for a mirror effect.
    """
